...
public class LockedDoor : MonoBehaviour, ISwappable {
    static private LockedDoor[,] _LOCKED_DOORS; // 2D array to store locked doors
    ...
    public Vector2Int mapLoc; // Coordinates on MapInfo.MAP where this tile is

    /// <summary>
    /// This public DoorInfo class is defined *inside* of the LockedDoor class.
    /// </summary>
    public class DoorInfo {
        public int tileNum;              // Property for the tileNum of the door Sprite
        public Vector2Int otherHalf;     // Property for the relative location of the otherHalf door

        /// <summary>
        /// Constructor for the DoorInfo class constructs a new DoorInfo
    ...
    void Start() {
        if (_LOCKED_DOORS == null) { // Initialize _LOCKED_DOORS if it's null
            BoundsInt mapBounds = MapInfo.GET_MAP_BOUNDS();
            _LOCKED_DOORS = new LockedDoor[mapBounds.size.x, mapBounds.size.y];
            InitDoorInfoDict();
        }

        mapLoc = Vector2Int.FloorToInt(transform.position); // Set mapLoc based on the position
        _LOCKED_DOORS[mapLoc.x, mapLoc.y] = this; // Store this locked door in the array
    }
    ...
    void InitDoorInfoDict() {
        _DOOR_INFO_DICT = new Dictionary<int, DoorInfo>(); // Create a new Dictionary

        // Create instances of DoorInfo and add them to the dictionary
    ...
    /// <returns>An int [0..3] representing the facing required
    ...
    /// <returns>The LockedDoor at that location or null
    ...
    public GameObject guaranteedDrop { get; set; } // Property for a guaranteed drop
    
    public int tileNum { get; private set; } // Property to get the tile number
    ...
    public void Init(int fromTileNum, int tileX, int tileY) {
        ...
        sRend.sprite = TilemapManager.DELVER_TILES[fromTileNum].sprite; // Set the sprite based on the tile number
    
        // Position this GameObject correctly
        transform.position = new Vector3(tileX, tileY, 0) + MapInfo.OFFSET; // Set the position of this GameObject
    }
}
